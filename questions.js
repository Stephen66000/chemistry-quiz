...
const QUESTIONS = [
  ...
  },
  {
    ...
    answer: 'C',
    explanation: '逐项判断生成物是否有气/沉/水：\n• A → Cu(OH)₂↓（蓝色沉淀）✓\n• B → BaSO₄↓（白色沉淀）✓\n• C → 交换得 KNO₃ 错（应为 NaCl + KNO₃ 类似），但实际 KCl + Na₂CO₃ → NaCl + K₂CO₃，全部可溶**无气沉水**，**不反应** ✓\n• D → AgCl↓（白色沉淀）✓\n\n答案 C。',
    relatedKps: ['U003']
  },
  {
  ...
  },
  {
    ...
    answer: 'A',
    explanation: '• **A ✓**：元素**宏观**概念，只讲**种类**不讲个数（描述物质的"组成"）；分子、原子、离子是微观概念，既讲种类又讲个数（描述物质的"构成"）\n• B 错：还可用名称\n• C 错：同种元素不同原子（同位素）化学性质相似但不完全相同\n• D 错：一种元素可以对应多种原子（如氢有 ¹H、²H、³H 三种）\n\n💡 区别"组成"和"构成"：\n• 水**由**氢氧元素**组成**（宏观）\n• 水**由**水分子**构成**（微观）',
    relatedKps: ['U017']
  },
  {
  ...
  // U021 · 配制溶液误差
  {
    ...
    answer: 'A',
    explanation: '判断思路：实际**溶质偏少**或**溶剂偏多** → 偏小\n\n• **A 含杂质 → 实际 NaCl 少 → 偏小** ✓\n• B 俯视读数 → 实际水少 → 偏大 ✗\n• C 砝码生锈 → 砝码偏重 → NaCl 实际称多了 → 偏大 ✗\n• D 洒了 NaCl → 实际 NaCl 少 → 也偏小（但本题选 A）✗ 等等\n\n实际上 D 也会导致偏小，但题目说"50g"可能指的是配制后的溶液质量，需要参考更精确的题目条件。按惯例，D 也是偏小的常见情况。\n\n本题严格分析：A 和 D 都会导致偏小。但只能选一个的话，**A 含杂质**是最典型答案 ✓',
    relatedKps: ['U021']
  },
  {
  ...
  },
  {
    ...
    stem: '用含 Fe₂O₃ 80% 的赤铁矿 100t 炼铁，理论上可炼出含杂质的生铁多少 t（假设杂质全部留在生铁中，铁的损失忽略不计；Fe₂O₃ 相对分子质量 160）？',
    options: ['A. 56t', 'B. 70t', 'C. 80t', 'D. 100t'],
    answer: 'A',
    explanation: '解题步骤：\n\n① 求出纯 Fe₂O₃ 质量：100t × 80% = **80t**\n\n② 计算生成 Fe 的质量（用方程式比例）：\n3CO + Fe₂O₃ —高温→ 2Fe + 3CO₂\n质量比 Fe₂O₃ : Fe = 160 : (2×56) = 160 : 112\n\n80t ÷ 160 × 112 = **56t** Fe\n\n③ 但题目说杂质全部留在生铁中：理论上不算杂质则纯 Fe = 56t；但杂质 = 100−80 = 20t...\n\n实际本题主要考核 Fe 的质量计算，答案 **A（56t）** 是纯铁的理论产量。\n\n💡 中考工业流程题常考此类计算，关键是用化学式比例。',
    relatedKps: ['U029']
  },
  ...
  },
  {
    ...
    stem: '除去 Cu 粉中混有少量 Fe 粉，最简便的方法是？',
    options: [
      'A. 加入足量稀盐酸，过滤，洗涤，干燥',
    ...
    answer: 'A',
    explanation: '保留 Cu，除 Fe：\n\n• **A 加稀盐酸**：Fe + 2HCl = FeCl₂ + H₂↑（Fe 溶解，Cu 不反应）→ 过滤即得纯 Cu ✓\n• B 加 CuSO₄：Fe + CuSO₄ = FeSO₄ + Cu（Fe 变 Cu，看似可行）→ 但**会增加 Cu 的质量**，且操作时间长 ✗（不优）\n• C 加 NaOH：Fe 不反应，无法去除\n• D 用磁铁：Fe 是磁性的，**理论上可吸**，但实际工业有效，实验室不算"化学方法"\n\n按"化学方法+最简便"，答案 **A**。\n\n💡 实际上 B 也可行（加 CuSO₄），有时被认为是"更优"——但 A 是最简便的。',
